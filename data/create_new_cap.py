...
def get_img_caption_json(dataset_root, clothing_type, split):
    with open(os.path.join(dataset_root, 'captions/raw_cap/', 'cap.{}.{}.json'.format(clothing_type, split))) as json_file:
    ...
    with open(os.path.join(dataset_root, 'captions/raw_cap/new_cap/', 'cap.{}.{}.json'.format(clothing_type, split)), 'a') as f:
